...
const observe = (target: Tms, targetPrototype: object, dispatch: Function) => {
    const descriptors: GetOwnPropertyDescriptors = getDescriptors(
        getPrototypes(target, targetPrototype)
    );
        ...
        if (
            ...
            Object.defineProperty(target, type, {
                ...
                value(...payloads: Array<any>) {
                    const value = descriptor.value.apply(
                        this ?? target,
                        payloads
                    );
